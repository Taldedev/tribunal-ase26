...
|---|---|---|
| **Whether the bench divides on genuinely different models** | **Measured, and it did not.** Three deliberations on T-001: two on arrangement A, unanimous both times, and one on B with the judges on three models — the two that ruled, on two different providers, agreed, and the third timed out. So the arrangement is verified and the *claim* it exists to test did not reproduce on this case. What would settle it is seven distinct models and a harder question, and seven distinct models are paid ones: three of nineteen free models answered on the day. Read the three readings of that result in [`coordination.md`](coordination.md) | a paid tier, or a genuinely contested charge sheet — neither available to this work |
